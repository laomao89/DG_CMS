...
function sys_ForShowSonClass($classid,$tempid,$show=0,$cline=0){
	...
	$no=1;
	$sql=$empire->query("select classid,classname,bname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}enewsclass where ".$where." and showclass=0 order by myorder,classid".$limit);
	while($r=$empire->fetch($sql))
	{
		//显示栏目数据数
		if($show)
...
function sys_GetShowClassMore($bclassid,$bcr,$tr,$bno,$bnum,$show=0){
	...
	$no=1;
	$changerow=1;
	$sql=$empire->query("select classid,classname,bname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}enewsclass where bclassid='$bclassid' and showclass=0 order by myorder,classid");
	while($r=$empire->fetch($sql))
	{
		//显示栏目数据数
		if($show)
